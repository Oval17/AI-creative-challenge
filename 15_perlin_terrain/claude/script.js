...
// ── Terrain grid ───────────────────────────────────────────────
const GRID_W = 40;   // columns
const GRID_H = 32;   // rows
const CELL   = 24;   // spacing between grid points (world units)

// Isometric projection
// World: x right, y up (height), z into screen
// Screen: iso_x = (x - z) * cos30, iso_y = (x + z) * sin30 - y
const ISO_SCALE = 1.0;
function project(wx, wy, wz) {
  const angle = Math.PI / 6; // 30°
  const sx = (wx - wz) * Math.cos(angle) * ISO_SCALE;
  const sy = (wx + wz) * Math.sin(angle) * ISO_SCALE - wy;
  return { sx, sy };
}

// ── Animation ──────────────────────────────────────────────────
let zOffset = 0;       // scroll the terrain forward
const SCROLL_SPEED = 0.006;

// Height scale and color
...
// Map height → color (deep blue ocean → teal → green → snow)
function heightColor(h, maxH) {
  const t = (h + maxH) / (2 * maxH); // 0=valley, 1=peak
  if (t < 0.3) {
    // Deep water: dark blue
    const tt = t / 0.3;
    return [Math.round(0 + tt*20), Math.round(30 + tt*80), Math.round(120 + tt*100)];
  } else if (t < 0.55) {
    // Shallow/shore: teal → green
    const tt = (t - 0.3) / 0.25;
    return [Math.round(20 + tt*20), Math.round(110 + tt*90), Math.round(220 - tt*140)];
  } else if (t < 0.78) {
    // Midlands: green → yellow-green
...
function render() {
  ...
  // Center the terrain on canvas
  const originX = CW / 2;
  const originY = CH * 0.72;

  // World coords: center grid
  ...
  for (let gz = 0; gz < GRID_H; gz++) {
    for (let gx = 0; gx < GRID_W; gx++) {
      ...
      // Wire edge — subtle bright lines
      const edgeBright = Math.min(255, r + 40);
